...
import { Terminal, Code2, Palette, Boxes, Database, Wifi, FlaskConical, Rocket, Package, MapIcon } from "lucide-react";
...
function CheatSheetSection({ section }: { section: CheatSection }) {
  return (
    <div className="space-y-2">
      <CopyableCodeBlock code={section.code} language={section.language} label={section.label} />
    </div>
  );
}
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="dart" className="w-full">
          <div className="overflow-x-auto -mx-1 px-1">
            <TabsList className="w-full min-w-max bg-white/5 border border-white/10">
              {categories.map((cat) => (
                <TabsTrigger
                  key={cat.id}
                  value={cat.id}
